...
### Finding Help with methods

```sh
db.todo.insertMany.help()
```
...
```sh
db.todo.insertOne({task: "Watch Tom and Jerry", completed: false})
```
...
```sh
db.todo.insertMany([
  {task: "Watch Tom and Jerry", completed: false},
  {task: "Watch Tom and Jerry - Reimagined", completed: false}
])
```
...
```sh
db.todo.find({"task": "watch Tom and Jerry"})
```

- Selecting the amount of data that is returned.

```sh
db.todo.find({"task": "Watch Tom and Jerry"}, {"task": 1})
```

> Returns only the _id and task, ignoring completed and any other value in the document.

```sh
db.todo.find({}, {"task": 1})
```

> Returns all documents with only the _id and task, ignoring completed and any other value in the document.

```sh
db.todo.find({ "task": "Watch John Wick 4"}, {task: 0})
```

> Returns other information about the document excluding the task.
...
> Returns the first two documents in the collection

- Skip

Allow you to skip the first few results

```sh
db.movies.find().skip(3)
```

> Skips the first 3 results
...
> By default mongodb generates an _id data in each document. One can override the data in the `_id` but one cannot change the key.
...
### Displaying data as tables on Mongosh

```sh
const results = db.newemp.find().toArray()
